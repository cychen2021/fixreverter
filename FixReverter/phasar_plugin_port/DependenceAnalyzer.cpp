...
map<DependenceAnalyzer::n_t, set<DependenceAnalyzer::d_t>>
DependenceAnalyzer::initialSeeds() {
  cout << "DependenceAnalyzer::initialSeeds()\n";
  map<DependenceAnalyzer::n_t, set<DependenceAnalyzer::d_t>> SeedMap;
  ...
  for (auto &EntryPoint : EntryPoints) {
    if (const auto *F = IRDB->getFunctionDefinition(EntryPoint)) {
      ...
      for (auto Global : GlobalVars)
        toGenerate.insert(FFManager.getOrCreateFlowFact(Global));
      // generate zero value
      toGenerate.insert(getZeroValue());
      SeedMap.insert(make_pair(&F->front().front(), toGenerate));
    } else {
